...
(defun parser-end-p (p) (>= (parser-pos p) (length (parser-text p))))

(defun peek (p &optional (offset 0))
  (let
    ((index (+ (parser-pos p) offset)))
    (and
      (< index (length (parser-text p)))
...
(defun whitespace-char-p (ch) (and ch (member ch '(#\space #\tab #\newline #\page #\return) :test #'char=)))

(defun skip-whitespace (p)
  (let
    ((newlines 0))
    (loop
      :while (and (not (parser-end-p p)) (whitespace-char-p (peek p)))
      :do
      (when
        (char= (peek p) #\newline)
        (incf newlines)
...
(defun opener-p (ch) (and ch (find ch "([{")))

(defun closer-p (ch) (and ch (find ch ")]}")))
...
(defun delimiter-char-p (ch) (or (opener-p ch) (closer-p ch)))
...
(defun parse-line-comment (p)
  (let
    ...
    (loop
      :while (and (not (parser-end-p p)) (not (char= (peek p) #\newline)))
...
(defun parse-character-literal-tail (p start-line start-col)
  (let
    ((next (peek p)))
    (when
      (or (null next) (whitespace-char-p next))
...
(defun parse-atom (p)
  (let
    ...
    (loop
      :while (and (not (parser-end-p p)) (not (whitespace-char-p (peek p))) (not (delimiter-char-p (peek p))) (not (char= (peek p) #\;)) (not (char= (peek p) #\")))
      :do
      (when
        (starts-with-p p "#|")
        (return)
...
(defun parse-prefix-chain (p)
  (let
    ...
    (when
      parts
      (let
        ((prefix (apply #'concatenate 'string (nreverse parts))))
        (skip-whitespace p)
        (when
          (and
            (>= (length prefix) 2)
...
(defun parse-list (p &optional prefix start-line start-col)
  (let*
    ...
    (let
      ((node (make-node :kind :list :children (nreverse children) :opener (string opener) :closer (string closer) :prefix prefix :start-line line :end-line (parser-line p) :start-col col)))
      (setf
        (node-kind node)
        (if
          (and
...
(defun parse-form (p)
  ...
  (let
    ((prefix (parse-prefix-chain p)))
    (when
      prefix
      (let
        ((child (first (node-children prefix))))
        (when
          (and child (member (node-kind child) '(:list :vector)))
          (let
            ((child-single-line-p (= (node-start-line child) (node-end-line child))))
            (setf
              (node-prefix child)
              (concatenate 'string (node-text prefix) (or (node-prefix child) ""))
              (node-start-line child)
              ...
              (node-start-col child)
              (node-start-col prefix)
            )
          )
          (when
            (and
              (plusp (length (node-prefix child)))
              (char=
                (char
                  (node-prefix child)
...
(defun parse-all (string)
  (let
    (
      (p (make-parser :text (normalize-input string)))
      (forms '())
      (gaps '())
    )
    (loop
      (let
        ((gap (skip-whitespace p)))
        (when
          (parser-end-p p)
          (return)
        )
...
(defun trim-right-spaces (string) (string-right-trim '(#\space #\tab #\newline #\return) string))
...
(defun ensure-comment-space (comment)
  (let*
    (
      (trimmed (trim-right-spaces comment))
      (len (length trimmed))
      (i 0)
    )
    (loop
      :while (and (< i len) (char= (char trimmed i) #\;))
      :do (incf i)
    )
    (cond
      ((= i len) trimmed)
...
(defun flush-block-comment-text (out pending)
  (let
    ((text (string-right-trim '(#\space #\tab #\newline #\return) (string-left-trim '(#\newline #\return) (get-output-stream-string pending)))))
    (when
      (plusp (length text))
  ...
  (let
    ...
    (and
      ...
      (loop
        :for index
        :below prefix-length
        :always (funcall test (char prefix index) (char string index))
      )
    )
  )
)
...
(defun loop-operator-p (node) (atom-text= node "loop"))

(defparameter +loop-keyword-names+ '("named" "with" "and" "initially" "finally" "do" "doing" "return" "collect" "collecting" "append" "appending" "nconc" "nconcing" "count" "counting" "sum" "summing" "maximize" "maximizing" "minimize" "minimizing" "into" "if" "when" "unless" "else" "end" "while" "until" "repeat" "always" "never" "thereis" "for" "as" "from" "upfrom" "to" "upto" "below" "by" "downto" "above" "downfrom" "in" "on" "then" "across" "being" "each" "the" "hash-key" "hash-keys" "of" "using" "hash-value" "hash-values" "symbol" "symbols" "present-symbol" "present-symbols" "external-symbol" "external-symbols" "of-type" "it")
)
...
(defun loop-keyword-text (node) (concatenate 'string ":" (atom-final-name node)))

(defun loop-keyword-name-in-p (node names)
  (let
    ((node-name (atom-final-name node)))
...
(defun definition-name-p (node)
  (or
    (member (node-kind node) '(:list :vector))
    (and
      (eq (node-kind node) :atom)
...
(defun definition-head-count (children)
  (when
    ...
    (if
      (and
        (not (atom-text= (first children) "define"))
...
(defun multiline-string-p (s) (find #\newline s))
...
(defun inline-safe-node-p (node)
  (and
    (not (node-forces-multiline-p node))
...
(defun list-multiline-p (node)
  (or
    (/= (node-start-line node) (node-end-line node))
...
(defun has-standalone-line-comment-p (children)
  (loop
    ...
    :for child
    :in children
    :thereis (and (eq (node-kind child) :line-comment) (not (trailing-line-comment-p previous child)))
  )
)

(defun indent-string (indent) (make-string indent :initial-element #\space))
...
(defun loop-line-item (node rewrite-p) (cons node rewrite-p))

(defun loop-line-item-node (item) (if (consp item) (car item) item))

(defun loop-line-item-rewrite-p (item) (and (consp item) (cdr item)))
...
(defun consume-loop-item (line-items rest rewrite-p)
  (if
    rest
    (values
      (append line-items (list (loop-line-item (first rest) rewrite-p)))
      (rest rest)
      (first rest)
    )
    (values line-items rest (loop-line-item-node (first (last line-items))))
...
(defun loop-binding-marker-p (node) (loop-keyword-name-in-p node '("for" "as" "with" "and")))

(defun loop-name-marker-p (node) (loop-keyword-name-in-p node '("named")))

(defun loop-value-marker-p (node)
  (loop-keyword-name-in-p
    node
    '(
      "in"
      "on"
...
(defun loop-body-marker-p (node) (loop-keyword-name-in-p node '("do" "doing" "initially" "finally")))

(defun loop-value-start-marker-p (node)
  (or
    (loop-value-marker-p node)
    ...
    (cond
      ...
      (
        ...
        (let
          ((last-node child))
          (loop
            :while (and rest (not (loop-keyword-atom-p (first rest))) (inline-safe-node-p (first rest)))
            :do
            (setf
              last-node
              (first rest)
...
(defun indent-multiline-string (string indent)
  (let
    ((indentation (indent-string indent)))
    (with-output-to-string
      (out)
      (loop
        :for line
        :in (split-lines string)
        :for index
        :from 0
        :do
        (when
...
(defun write-child-lines (children indent out &key loop-style group-keywords previous)
  (loop
    ...
    (multiple-value-bind
      ...
      (let
        ((line (if loop-style (format-loop-child-line line-nodes indent) (format-child-line line-nodes indent))))
        (if
          (and
            previous
        ...
        (write-string
          (if
            (rendered-line-needs-context-indent-p line-nodes)
    ...
    (
      ...
      (if
        ...
        (let
          ...
          (if
            (eq (node-kind child) :block-comment)
...
(defun format-list-node (node indent &key inline force-multiline)
  (let*
    ...
    (cond
      ...
      (
        (and
          (eq (node-kind node) :list)
          (definition-head-count children)
        )
        (let
          ((head-count (definition-head-count children)))
          (let
            ((remaining (nthcdr head-count children)))
            (if
              (and
                (= (length remaining) 1)
